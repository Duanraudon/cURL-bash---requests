...
def dealwith_cookies(r):
    #re正则选取cookie字段
    cookie = re.findall('cookie.*?(?:\' )|Cookie.*?(?:\' )|cookie.*?(?:\')',r)
    #数据清洗打印
    try:
        if cookie != '[]':
...
def dealwith_headers(r):
    ...
    authority = dealwith_parameter('Authority',r)
    x_requested_with = dealwith_parameter('x-requested-with',r)
    headers = "headers = {\n"+Origin+Accept_Encoding+Accept_Language+Upgrade_Insecure_Requests+\
              User_Agent+Content_Type+Accept+authority+x_requested_with+Cache_Control+Referer+Connection+"}"+'\n'
    print(headers)
...
if __name__ == '__main__':
    ...
    cookies = dealwith_cookies(r)
    data = dealwith_data(r, datas = [])
    dealwith_headers(r)
    #url选取清理
